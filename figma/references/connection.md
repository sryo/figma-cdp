...
How to get a working `figma` global for Plugin API access. Three options, from simplest to most manual.

## Option A: Claude in Chrome MCP (preferred)

If the `mcp__claude-in-chrome__*` tools are available, use `mcp__claude-in-chrome__javascript_tool` to eval JS against the Figma tab. No setup required.
...
2. Ensure the page has **fully loaded** — wait a few seconds and retry.
3. Ensure you are evaluating in the **correct tab/frame** — check the URL matches the Figma file.
4. If the global is still missing, instruct the user to **open and close any plugin** (this initializes the Plugin API), then retry.
